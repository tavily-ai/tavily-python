...
class AsyncTavilyClient:
    ...
    def __init__(
        ...
        self._client_creator = lambda: httpx.AsyncClient(
            headers={"Content-Type": "application/json", "Authorization": f"Bearer {api_key}", "X-Client-Source": "tavily-python"},
            base_url=self._api_base_url,
            mounts=(
                {scheme: httpx.AsyncHTTPTransport(proxy=proxy, verify=verify) for scheme, proxy in mapped_proxies.items()}
                if mapped_proxies
                else None
            ),
        )
