...
def _validate_index(client):
    ...
    for index in client.collection.list_search_indexes():
        ...
        if index["type"] != "vectorSearch":
            raise ValueError(
                f"Index '{client.index}' exists but is not of type 'vectorSearch'."
            )
        ...
        for field in index["latestDefinition"]["fields"]:
            ...
            if field["type"] != "vector":
                raise ValueError(
                    f"Field '{client.embeddings_field}' exists but is not of type 'vector'."
                )
            elif field["similarity"] != "cosine":
                raise ValueError(
                    f"Field '{client.embeddings_field}' exists but has similarity '{field['similarity']}' instead of 'cosine'."
                )
        ...
        if not field_exists:
            raise ValueError(
                f"Field '{client.embeddings_field}' does not exist in index '{{client.index}}'."
            )
...
def _cohere_embed(texts, type):
    return co.embed( # type: ignore
        model="embed-english-v3.0", texts=texts, input_type=type
    ).embeddings


def _cohere_rerank(query, documents, top_n):
    response = co.rerank( # type: ignore
...
class TavilyHybridClient:
    def __init__(
        ...
        if db_provider != "mongodb":
            raise ValueError(
                "Only MongoDB is currently supported as a database provider."
            )
        ...
        self.embedding_function = (
            _cohere_embed if embedding_function is None else embedding_function
        )
        self.ranking_function = (
            _cohere_rerank if ranking_function is None else ranking_function
        )
    ...
    def search(
        ...
        **kwargs,
        ...
        query_embeddings = self.embedding_function([query], "search_query")[0] # type: ignore
        ...
        # Search using tavily
        if max_foreign > 0:
            foreign_results = self.tavily.search(
                query, max_results=max_foreign, **kwargs
            )["results"]
        else:
            foreign_results = []
        ...
        combined_results = self.ranking_function(
        ...
        if max_foreign > 0 and save_foreign != False: # noqa: E712
            ...
            embeddings = self.embedding_function(
                [result["content"] for result in foreign_results],
                "search_document",
            )
            for i, result in enumerate(foreign_results):
                result["embeddings"] = embeddings[i] # type: ignore
